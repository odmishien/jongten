from util import get_random_hand, get_random_kaze_set, get_is_or_not
...
def main():
    ...
    tiles = TilesConverter.string_to_136_array(
        man=hand['man'], pin=hand['pin'], sou=hand['pin'])
    win_tile = TilesConverter.string_to_136_array(**hand['win_tile'])[0]
    conf = {
        'is_tsumo': get_is_or_not(),
        'is_riichi': get_is_or_not(),
        'is_ippatsu': get_is_or_not(),
    ...
    question_caption = ''
    if conf['is_tsumo']:
        question_caption += f"ツモ {hand['win_tile']}"
    if conf['is_riichi']:
    ...
    if client_answer[0] == result.cost['main'] and client_answer[1] == result.cost['additional']:
        print('正解!!')
    else:
